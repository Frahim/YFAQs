...
class yfaqs_Options_Framework_Interface {

	/**
	 * Generates the tabs that are used in the options menu
	 */
	static function yfaqs_optionsframework_tabs() {
		$counter = 0;
	 ...
	 */
	static function yfaqs_optionsframework_fields() {
		...
		if ( isset( $yfaqs_optionsframework_settings['id'] ) ) {
			$option_name = $yfaqs_optionsframework_settings['id'];
		}
		else {
			$option_name = 'yfaqs_optionsframework';
		};
		...
		foreach ( $options as $value ) {
			...
			if ( has_filter( 'yfaqs_optionsframework_' . $value['type'] ) ) {
				$output .= apply_filters( 'yfaqs_optionsframework_' . $value['type'], $option_name, $value, $val );
			}
			...
			// Uploader
			case "upload":
				$output .= yfaqs_Options_Framework_Media_Uploader::yfaqs_optionsframework_uploader( $value['id'], $val, null );
			...
			case 'background':
				...
				$output .= yfaqs_Options_Framework_Media_Uploader::yfaqs_optionsframework_uploader( $value['id'], $background['image'], null, esc_attr( $option_name . '[' . $value['id'] . '][image]' ) );
				...
				$repeats = yfaqs_of_recognized_background_repeat();
				...
				$positions = yfaqs_of_recognized_background_position();
				...
				$attachments = yfaqs_of_recognized_background_attachment();
			...
			case "info":
				...
				if ( isset( $value['desc'] ) ) {
					$output .= apply_filters('yfaqs_of_sanitize_info', $value['desc'] ) . "\n";
				}
				$output .= '</div>' . "\n";
				break;
		...
		if ( yfaqs_Options_Framework_Interface::yfaqs_optionsframework_tabs() != '' ) {
			echo '</div>';
		}
	}
